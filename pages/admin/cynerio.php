...
try {
    ...
    $command = "cd /var/www/html && python3 python/services/ingest_cynerio.py";
    $output = shell_exec($command . ' 2>&1');

    if (json_last_error() === JSON_ERROR_NONE) {
        #echo json_encode(['success' => true, 'data' => $output]);
    } else {
        echo json_encode(['success' => false, 'error' => 'Failed to ingest cynerio data']);
    }
} catch (Exception $e) {
    echo json_encode(['success' => false, 'error' => 'Cynerio ingestion failed: ' . $e->getMessage()]);
}
?>
